...
```sh
npm install @ticora/core @ticora/react @ticora/ui zod
```
...
in your [dashboard](/reference/api#workspace-service-client) → **API keys**:

{/* hosted-api:start — the base URL is synced from apps/docs/hosted-api.json (hosted-api.test.ts) */}
```ts
...
- Put [`ticora contracts diff`](/guides/vendor-ci) in CI before you change a contract.
- Render in **your own components** so a generated workspace looks like your product — see [Bring your own components](/guides/host-components) (or just restyle ours with the [`--we-*` theme tokens](/reference/api#theming)).
