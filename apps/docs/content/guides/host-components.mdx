...
burndown, a heatmap — the validator has to learn it too, because it gates every
block's config and binding shape. That means a registry entry alongside your
component. See [`defineBlockType`](/reference/api) for the current surface.

Until you need one, prefer an existing type: `CasesTable` (rows),
`GroupedBoard` (groups), `KpiCards` (aggregates), `Graph`, `CaseQueue`,
`FilterBar`.
