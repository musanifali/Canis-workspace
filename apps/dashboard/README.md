...
public SDK surface only — `@ticora/react` + `ui` blocks +
...
```bash
npm run db:up -w @ticora/db        # dedicated PG :5443
npm run dev -w @ticora/api         # service :8270
node scripts/seed-dashboard-tenant.mjs       # tenant + key (repo root)
# → paste printed values into apps/dashboard/.env.local
npm run seed -w @ticora/dashboard  # contracts + views via /v1 only
npm run dev -w @ticora/dashboard   # http://localhost:3002
```
