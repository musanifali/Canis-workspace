...
export const Graph = React.forwardRef<HTMLDivElement, GraphProps>(
  (
    { className, variant, size, data, title, showLegend = true, ...rawProps },
    ref,
  ) => {
    // Tambo injects _tambo_* status params into generated props — strip them
    // so the {...props} spreads below don't put unknown attributes on the DOM
    const props = Object.fromEntries(
      Object.entries(rawProps).filter(([key]) => !key.startsWith("_tambo_")),
    );
